...
//From StockFish
inline uint64_t random_uint64_t() {
	seed ^= seed >> 12;
	seed ^= seed << 25;
	seed ^= seed >> 27;
...
inline uint64_t hash_pce(uint64_t zobrist_key, Piece pce, const int sq) {
	return zobrist_key ^ keys_pce[pce][sq];
}
inline uint64_t hash_castling_rights(uint64_t zobrist_key, const int castling_rights) {
	return zobrist_key ^ keys_castling_rights[castling_rights];
}
inline uint64_t hash_side_to_move(uint64_t zobrist_key) {
