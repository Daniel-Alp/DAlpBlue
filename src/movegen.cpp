...
void gen_pawn_moves(const Position& pos, MoveList& move_list, const uint64_t targets, const Color col) {
	...
	if (col == Color::WHITE) {
		const uint64_t pawns = pos.pce_bitboards[Piece::WHITE_PAWN];
		const uint64_t enemy = pos.col_bitboards[Color::BLACK];
		...
		const uint64_t pawns = pos.pce_bitboards[Piece::BLACK_PAWN];
		const uint64_t enemy = pos.col_bitboards[Color::WHITE];
